...
ИИ — опциональный ограниченный fallback, в рантайме не обязателен. Интерфейса два: своя
веб-страница `GET /` (`app/templates/index.html`, Яндекс.Карты) и Swagger UI (`/docs`);
оценка альтернативного фронта на C++ — `CPP_FRONTEND_OPTIONS.md`. pik.ru — только новостройки.
