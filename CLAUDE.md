...
tests/               # integration/ — E2E; parsing/ reference/ geo/ ai/; corpus/queries.txt; test_parse_audit_regression.py
...
scripts/             # parse_audit.py; cleanup_metro_duplicates.py
```
...
- **Экстрактор свободного текста (`resolve_free_text_criteria`) — ОСЛАБЛЕНИЕ гейтов
  ...
  **time_on_foot/time_on_transport** — страховка на случай, если детерминированная идиома
  ...
  `sanitize_*`). Инварианты: детерминированное всегда выигрывает (пишем только `None`-поля);
...
- **Замкнутый контур проверки:** `tests/corpus/queries.txt` (197 запросов),
  `scripts/parse_audit.py` (офлайн `parse`→`build_url` без сети/ИИ),
  `tests/test_parse_audit_regression.py` — гоняется в каждом `pytest` с порогами «не
  хуже»: crashes=0, empty≤31, no-filter-URL≤63, coverage≥0.89 (ужесточены после «ведра A»
  — детерминированных добивок опечаток/синонимов; факт 27/60/0.914 после дочинки живого
  прогона — числительные-комнаты, whitebox, медицина-POI, вузы). **Ужесточать по мере
  улучшений; ослаблять только с обоснованием.**
