...
def test_live_incident_is_split_by_category():
    ...
    Тест держится за живое поведение парсера, и предсказание прежней редакции
    сбылось: примером `lost` служил «этаж от 7», но правила этажа научились
    инверсии («этаж» ПЕРЕД числом), фрагмент доезжает до ссылки как `floor_min`
    и остатком быть перестал. Это «остаток закрыт», а не поломка разметки —
    инвариант границы держат `test_residual_*` выше, они от правил парсинга не
    зависят. Пример заменён на ещё не закрытый; когда закроют и его, поступать
    так же.
    """
    result = parse("Привет, ищу двушку до 15 млн, высота потолков 3.2 м")

    by_category = {category_of(w): str(w) for w in result.warnings}
    assert by_category[WarningCategory.NOISE].startswith("«Привет»")
    assert by_category[WarningCategory.LOST].startswith("«высота потолков 3.2 м»")
    ...
    assert [str(w) for w in result.warnings] == [
        ...
        "«высота потолков 3.2 м»: не удалось распознать, не попало в ссылку",
    ]
...
def client(monkeypatch):
    # ИИ выключаем явно: остаток дотягивается до free-text-экстрактора и звал бы
    # живую модель. Здесь проверяется канал warnings, не ИИ.
...
def test_build_url_response_carries_detailed_warnings(client):
    response = client.post(
        "/build-url",
        json={"text": "Привет, ищу двушку до 15 млн, высота потолков 3.2 м"},
    )
    ...
    incident_lost = "«высота потолков 3.2 м" + _UNRECOGNIZED_SUFFIX
