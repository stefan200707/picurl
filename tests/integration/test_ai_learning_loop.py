...
def memory_db(monkeypatch):
    ...
    async def fake_lookup_structured_fact(pool, subject_type, subject_id, fact_type):
        for f in db.structured_facts:
            if f.subject_type == subject_type and f.subject_id == subject_id and f.fact_type == fact_type:
                return f
        return None

    async def fake_store_structured_fact(pool, subject_type, subject_id, fact_type, value, source, confidence):
...
def test_ai_learning_loop_e2e(e2e_client, mock_anthropic_client, memory_db, monkeypatch):
    ...
    from app.reference.loader import load_complexes, clear_cache
    clear_cache()
    complexes = load_complexes()
    valid_complex_id = complexes[0].id if complexes and complexes[0].id else "test_id"
    valid_complex_slug = complexes[0].slug if complexes and complexes[0].slug else "test_slug"
    ...
    text2 = "хочу три комнаты у центра, чтобы рядом сад и школа были"
    # To hit cache in our simple fake DB, the signature must match.
    # Since signature is based on semantic leftover + candidates, it should match if criteria match exactly.
    # In reality, signature matching is done by pgvector, but our mock needs exact signature.
    # Let's just use the exact same text to guarantee mock hit, or we can monkeypatch build_query_signature to return constant.
    ...
    # No, promote uses asyncpg.Pool. We need to mock update queries or just call promote with fake pool.
    import asyncio
