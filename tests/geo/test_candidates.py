...
def test_poi_only_new_is_not_fully_resolved():
    """«Только новые» кэш не различает — факт не подтверждён детерминированно,
    решение остаётся за ИИ.

    Схема v2 отделила действующие объекты от строящихся, но «новизна» открытого
    сада в OSM-тегах не выражена вовсе — неопределённость сохраняется."""
    from app.geo.poi import POICategory
