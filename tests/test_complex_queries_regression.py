...
Фактический замер после AI-27 (слот `_FILLER` в пешем MODE — «до метро БЫЛО идти»),
корпус 34 запроса: crashes=0, URL-без-фильтров=0, без blocks=8, coverage=0.949,
фильтров на запрос=9.06. Предыдущие замеры: AI-26 (33 запроса, зазор ведущей
POI-дистанции + «этаж от N») 0 / 0 / 8 / 0.947 / 9.06; AI-25 (31 запрос, guard
«безметровых» форм пешего времени, «не больше» в маркере дистанции, ведущая
дистанция прижата к границе слова, хвостовая дистанция раздаётся перечислению,
«отделка под ключ», честный warning валидатора) 0 / 0 / 7 / 0.9330 / 8.94; AI-24
(31 запрос) 0 / 0 / 7 / 0.9282 / 9.03; ещё раньше (30 запросов) 0 / 0 / 9 / 0.899 /
8.87.

ВНИМАНИЕ: у ``MAX_WITHOUT_BLOCKS`` сейчас нулевой запас (8 из 8). Новый запрос
корпуса без гео-якоря упрётся в порог — это сигнал усилить запрос, а не планку.
"""
