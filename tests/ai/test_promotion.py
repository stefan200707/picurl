...
from app.ai.promotion import promote, PROMOTION_MIN_OBSERVATIONS, PROMOTION_MIN_CONFIDENCE
...
async def test_promote_facts(temp_data_dir):
    with patch("app.ai.promotion.DATA_DIR", temp_data_dir):
        ...
        facts = [
            StructuredFact(
                id=1, subject_type="county", subject_id="1", fact_type="is_center",
                fact_value={"is_center": True}, source="ai_inference",
                confidence=0.9, observed_count=4, first_seen_at=datetime.now(), last_confirmed_at=datetime.now()
            ),
            StructuredFact(
                id=2, subject_type="complex", subject_id="100", fact_type="poi_school",
                fact_value={"present": True}, source="ai_inference",
                confidence=0.9, observed_count=4, first_seen_at=datetime.now(), last_confirmed_at=datetime.now()
            ),
            StructuredFact(
                id=3, subject_type="district", subject_id="99", fact_type="is_center",
                fact_value={"is_center": True}, source="ai_inference",
                confidence=0.9, observed_count=4, first_seen_at=datetime.now(), last_confirmed_at=datetime.now()
            ),
        ]
...
async def test_resolve_known_facts_uses_cache(temp_data_dir):
    with patch("app.geo.candidates.DATA_DIR", temp_data_dir), \
         patch("app.reference.loader.DATA_DIR", temp_data_dir):
        
        temp_data_dir.joinpath("complexes.json").write_text(json.dumps([{"name": "ЖК", "slug": "slug1", "id": "1"}]))
        temp_data_dir.joinpath("poi_cache.json").write_text(json.dumps({
            "slug1": {"school": {"count": 1, "closest_distance_m": 100}}
        }))
        ...
        criteria = Criteria(poi_requirements=[POIRequirement(category=POICategory.SCHOOL, raw_phrase="школа")])
        candidates = [
            ComplexCandidate(id="1", name="ЖК", district=None, county=None, metro=[], is_center=None, known_poi={"school": True})
        ]
