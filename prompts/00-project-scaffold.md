...
3. **`app/main.py`** — минимальный FastAPI-app с health-заглушкой и
   ...
   У `FastAPI(...)` сразу задай осмысленные `title`/`description`/`version` —
   единственный интерфейс пользователя будет автосгенерированная **Swagger-форма
   на `/docs`** (своего фронтенда у сервиса нет), поэтому её не отключаем и делаем
   информативной с самого каркаса.
4. **`README.md`** проекта: краткое описание, как установить (`uv sync`), как
   запустить (`fastapi dev` / `uvicorn`), как гонять тесты (`pytest`),
   как линтовать (`ruff check` / `ruff format`). Явно укажи, что вводить текст и
   получать ссылку удобнее всего через **Swagger-форму на `http://127.0.0.1:8000/docs`**
   (Try it out → заполнить `text` → Execute → JSON со ссылкой) — отдельный фронт
   не нужен.
