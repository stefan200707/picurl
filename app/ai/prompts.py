...
СТРОГИЕ ПРАВИЛА:
1. НИКОГДА не извлекай метро, районы, округа, ЖК, опции (санузлы, окна, отделка
   и т.п.) — этим занимаются другие слои. Если фрагмент про них — просто пропусти
   его. Ориентиры (landmarks) — исключение, но строго по каталогу, см. выше.
