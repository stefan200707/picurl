...
#: Минимальный QRatio окна против алиаса для options/option_groups. WRatio
#: учитывает partial_ratio и завышает оценку, когда МНОГОСЛОВНОЕ окно содержит
#: имя опции лишь как подстроку одного своего токена, а прочие слова — из другого
#: пожелания (окно перескочило союз «и»): «панорамными окнами и видом» даёт
#: WRatio 85.5 против алиасов «с видом на воду»/«видом на парк» (partial:
#: «видом»→«видом на…»), хотя это разные пожелания. Строгий QRatio-гейт ниже
#: исторически применялся лишь к ОДНОсловным option-окнам; двусловные+ пропускались,
#: и такой «и»-мостик просачивался. QRatio (полная строка, с учётом порядка)
#: разделяет этот класс чисто и с большим зазором: ложные ≤39 («видом на парк»=30.8,
#: «с видом на воду»=39) vs истинные ≥66 («во двор»→«Вид во двор»=77.8, «двумя
#: санузлами»→«2 санузла»=66.7, «сквозным санузлом»=78.8). Порог 55 — посередине
#: зазора, с запасом с обеих сторон. Мягче основного STRICT_QRATIO_THRESHOLD (80),
#: чтобы НЕ терять истинные склонения/цифро-словные формы, но отсечь «и»-мостик.
...
def _score_window(window_tokens, text_before, start_idx, end_idx, is_synthetic, text, choices):
    ...
    for r in res:
        ...
        # Опции/группы опций: полная строка окна должна быть достаточно похожа на
        # алиас (QRatio), а не цепляться за него partial_ratio'ом одного токена
        # (см. коммент у OPTION_MIN_QRATIO). Ловит многословные окна, перескочившие
        # союз «и» («…окнами и видом» ~ «видом на парк»/«с видом на воду»), которые
        # строгий однословный QRatio-гейт ниже не покрывает.
        if (
            etype in ("options", "option_groups")
            and fuzz.QRatio(query_norm, matched_str) < OPTION_MIN_QRATIO
        ):
            continue
