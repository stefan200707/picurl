...
from app.reference.loader import load_all, RefEntry, normalize
...
STOP_WORDS = {
    "в", "на", "у", "с", "по", "и", "или", "для", "к", "от", "до", "за",
    "около", "рядом", "хочу", "ищу", "квартиру", "квартиры", "куплю",
    "мне", "нужна", "пожалуйста", "подскажите", "а", "но", "же", "только"
}
...
def match_entities(text: str) -> tuple[list[EntityMatch], list[str]]:
    ...
    for c in candidates:
        ...
        if len(best_matches) > 1:
            ...
            warnings.append(f"Неоднозначность для «{c['text']}»: выбрано {entry.name}, возможные варианты: {', '.join(alt_names)}")
