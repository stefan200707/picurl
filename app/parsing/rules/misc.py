...
#: Диапазон лет заселения. Две формы, которых не хватало и которые в живых
#: запросах встречаются постоянно:
#:
#: 1. Слово «год…» между первым годом и предлогом — «с 2026 ГОДА по 2029 год».
#:    Раньше обязательным был вплотную идущий предлог, поэтому такой запрос
#:    распознавался только наполовину (``from`` заполнялся правилом MIN, а «по
#:    2029 год» оседало в остатке) — верхняя граница молча терялась.
#: 2. Вилка через дефис/тире — «в 2026-2027 годах», «заселение 2026-2028».
#:    Здесь было ХУЖЕ, чем потеря: срабатывало правило EXACT на первом годе и
#:    выдавало ``from=to=2026``, то есть готовый URL-фильтр, ПРОТИВОРЕЧАЩИЙ
#:    запросу — он отсекал как раз те годы, которые пользователь просил.
#:
#: Порядок проверки в :func:`extract_settlement_year` (RANGE раньше EXACT) —
#: обязателен: иначе EXACT снова заберёт первый год вилки.
_SETTLEMENT_YEAR_RANGE = re.compile(
    r"\b(?:заселение|сдача|въезд)\s+(?:с\s+|в\s+)?(\d{4})(?:\s+год\w*)?"
    r"\s*(?:по|до|[-–—])\s*(\d{4})(?:\s+год\w*)?\b"
)
...
_UNSUPPORTED_CATALOG: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"\bвторичк\w*|\bвторичн\w+(?:\s+(?:рынок|рынке|рынка|жиль\w*|фонд\w*))?"),
