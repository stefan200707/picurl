...
# Инверсия «отделка готовая» — тот же приём, что у «чернов»/«чистов»/«предчистов»,
# где обратный порядок слов прописан явной веткой. Для «готовой» её не было, и
# фраза уходила в warnings + ``option_candidates`` (см. тот же дефект у «под ключ»).
# Вставка «(?:уже\s+)?» покрывает «отделка уже готовая»; шире зазор не делаем —
# произвольное слово между «отделкой» и «готовой» рискует склеить разные требования.
_FINISH_TRUE = re.compile(
    r"\bс\s+отделк\w+|\bс\s+ремонт\w+|\bчистов\w+(?:\s+отделк\w+)?|\bотделк\w*\s+чистов\w+"
    r"|\bготов\w+\s+отделк\w+|\bотделк\w*\s+(?:уже\s+)?готов\w+"
    r"|\b(?:с\s+)?отделк\w*\s+под\s+ключ\b|\bпод\s+ключ\b"
)
