...
#: Необязательный ВЕДУЩИЙ носитель факта: «ЭТАЖНОСТЬЮ от 9 до 16 этажа». Слово
#: несёт ровно тот факт, который правило и извлекает, поэтому обязано входить в
#: consumed-span — иначе оседает в остатке ложным warning'ом «не удалось
#: распознать» про уже распознанное (инвариант 1). Группа незахватывающая, номера
#: групп 1/2 не меняются, спан растёт только влево. В `_UNSUPPORTED_CATALOG`
#: «этажность» намеренно НЕ вносится: факт поддержан URL-схемой (floorFrom/
#: floorTo), объявить его неподдерживаемым было бы неправдой.
_ETAZHNOST = r"(?:\bэтажност\w*\s+)?"

_FLOOR_RANGE_A = re.compile(
    rf"{_ETAZHNOST}\b(?:с|от)\s+(\d+)\s+(?:по|до)\s+(\d+)\s*(?:-?го)?\s*этаж\w*"
)
