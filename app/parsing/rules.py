...
_PRICE_BUDGET = re.compile(
    rf"\b(?:бюджет\w*|цена|стоимость\w*)\s*[—:\-]?\s*(до|от|не\s+более)?\s*({_NUM})\s*"
    rf"({_PRICE_UNIT}|м|m|к|k)?{_OPT_RUB}(?![\w²])"
)
_PRICE_MIN = re.compile(
    rf"\b(?:от|не\s+дешевле|не\s+менее|минимум)\s+({_NUM})\s*({_PRICE_UNIT}){_OPT_RUB}(?![\w²])"
)
_PRICE_MAX = re.compile(
    rf"\b(?:до|не\s+дороже|не\s+больше|не\s+более|максимум|в\s+пределах)\s+({_NUM})\s*"
...
_PRICE_PLAIN_MAX = re.compile(
    rf"\b(?:до|не\s+дороже|не\s+больше|не\s+более|максимум)\s+({_NUM}){_OPT_RUB}\b"
)
